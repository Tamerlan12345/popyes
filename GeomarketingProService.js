...
class GeomarketingProService {
    ...
    static async findBestLocationsInBounds(bounds) {
        ...
        // 1. Construct Query for Magnets (Malls, Universities, Subway Entrances)
        const query = `
          [out:json][timeout:25];
          (
        ...
        if (typeof window._executeOverpassQuery !== 'function') {
            throw new Error("_executeOverpassQuery global function not found");
        }

        const data = await window._executeOverpassQuery(query, "FindMagnets");
        ...
        data.elements.forEach(el => {
            ...
            let type = "Локация";
            let score = 50;
            let name = tags.name || tags['name:ru'] || tags['name:en'] || "Безымянный объект";
            ...
            candidates.push({
                lat, lng: lon, type, name, estimatedScore: score,
                id: el.id
            });
        });

        // Sort by score
        candidates.sort((a, b) => b.estimatedScore - a.estimatedScore);

        // Deduplicate (300m)
        const uniqueCandidates = [];
        for (const cand of candidates) {
            let tooClose = false;
            for (const kept of uniqueCandidates) {
                const dist = this.getDistKm(cand.lat, cand.lng, kept.lat, kept.lng);
                if (dist < 0.3) {
                    tooClose = true;
                    break;
                }
    ...
    static getDistKm(lat1, lon1, lat2, lon2) {
        const R = 6371;
